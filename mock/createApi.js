...
module.exports = (app) => {
  ...
  apiList.forEach((api) => {
    ...
    const middleware = apiConfig.middleware;
    ...
    const routeHandle = function(req, res) {
    ...
    if (middleware) {
      app.use([route, `${route}/:id`], middleware, routeHandle);
    } else {
      app.use([route, `${route}/:id`], routeHandle);
    }
  });
  return showList;
};
